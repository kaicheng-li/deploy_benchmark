...
echo ""
echo "  使用方式:"
echo "    转换模型:  cd openvino && python convert_model.py --config config.yaml"
echo "    推理:      cd openvino && python inference.py --config config.yaml --prompt 'Hello'"
echo "    CV 推理:   cd openvino && python inference.py --task image-classification --image cat.jpg"
echo "    基准测试:  cd openvino && python benchmark.py --config config.yaml"
echo "    C++ 基准:  cd openvino/build && ./ov_benchmark --model ../openvino_models/model.xml""
