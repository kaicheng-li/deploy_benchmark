...
echo ""
echo "  使用方式:"
echo "    导出模型:  cd onnx && python export_onnx.py --config config.yaml"
echo "    推理:      cd onnx && python inference.py --config config.yaml --prompt 'Hello'"
echo "    基准测试:  cd onnx && python benchmark.py --config config.yaml"
