...
- 模型权重占用约 16.69GiB；
- 推理初始化阶段分配约 2.97GiB KV Cache；
- RTX 5090 总显存约 32GB；
- 压测期间 GPU 显存峰值约 21,974MiB（约 21.5GiB）。
...
测试条件：同一张图片、同一个中文 prompt、`temperature=0`、`top_p=1`、`max_tokens=32`，请求使用 `stream=true`；每个并发级别先预热 10 次，再测量 100 次。服务端 `max_batch_size=2`，客户端并发级别为 1、2、4。

| 并发数 | 成功率 | TTFT 平均/P50/P95/P99 | TPOT 平均/P50/P95/P99 | E2E 平均/P50/P95/P99 | 请求吞吐 | 输出吞吐 | GPU 峰值 |
|---:|---:|---|---|---|---:|---:|---:|
| 1 | 100/100 | 12.632/12.466/13.292/13.505 s | 16.81/16.82/16.87/16.92 ms | 13.154/12.988/13.815/14.024 s | 0.076 req/s | 2.433 tok/s | 21,974 MiB |
| 2 | 100/100 | 25.182/25.064/26.108/26.556 s | 18.12/17.75/19.05/19.30 ms | 25.744/25.626/26.655/27.132 s | 0.078 req/s | 2.486 tok/s | 21,974 MiB |
| 4 | 100/100 | 44.333/44.703/51.945/52.332 s | 216.60/17.70/436.93/444.51 ms | 51.048/51.266/52.988/53.312 s | 0.078 req/s | 2.488 tok/s | 21,974 MiB |
...
- **TTFT**：从请求开始到首个非空流式内容 chunk 的时间。
- **TPOT**：首个内容 chunk 到最后一个内容 chunk 的时间除以输出 token 数减一。
- **请求吞吐**：完成请求数除以本轮墙钟耗时。
- **输出吞吐**：输出 token 总数除以本轮墙钟耗时。

完整原始结果：`results/tensorrt_qwen3vl_stream_benchmark_20260811_162326.{json,csv,md}`。

## 8. 延迟为什么高

当前 Qwen3-VL 的慢点主要在 **TTFT/预填充**，不是生成阶段。一次请求的时间可以拆成：

```text
E2E = 图片读取与预处理
    + Vision Encoder
    + 图文 token 拼接与语言模型 prefill
    + 第一个输出 token
    + 后续 token 解码
```

本次正式流式压测中，并发 1 的 TTFT 平均为 12.63 秒、P95 为 13.29 秒；后续 token 间隔约为十几毫秒。这说明视觉编码、图文 prefill 和首 token 准备占主要时间，单纯提高生成 kernel 速度不会显著降低首 token 延迟。

造成当前结果偏慢的具体因素：

1. Qwen3-VL-8B 每个请求都要运行视觉塔，再进行语言模型 prefill；纯文本 LLM 不需要这一段。
2. 图片会被处理成视觉 token；本次服务返回的 prompt token 数为 487，输入并不是一个很短的文本请求。
3. 本机为 RTX 5090 32GB，服务端为避免 OOM 将最大活动 batch 限制为 2；客户端并发 4 时会出现排队，单请求 E2E 平均升至约 51.05 秒。
4. 当前使用 PyTorch backend，服务启动时需要加载 16GB 级别权重、做 autotuner/CUDA Graph warmup，并在运行时处理动态多模态输入；它没有预先构建好的固定 engine 来消除这些运行时步骤。
5. 本次统一要求 `stream=true`、`max_tokens=32`，流式只改变返回方式，不会跳过视觉编码和 prefill，所以不会自动降低 TTFT。

因此优化优先级应是：先确认 Vision Encoder 和 prefill 的耗时，再考虑降低图片视觉 token 数、使用 FP8/INT8 权重、换更大 GPU 或采用具备稳定 Qwen3-VL engine conversion 的版本；不能只盯着 TPOT。
...
1. 当前基准已使用流式 token 时间戳统计 TTFT 和 TPOT；仍应在目标业务 prompt 分布上复测。
2. 压测期间 GPU 显存峰值约 21,974MiB；并发继续提高时，服务端 `max_batch_size=2` 会造成排队，显存和延迟仍需单独评估。
3. 当前每个并发级别测量 100 个请求，足以验证当前配置的稳定性，但仍不足以替代长期容量规划。
